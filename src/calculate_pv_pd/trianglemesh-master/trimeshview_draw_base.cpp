...
void TriMeshView::compute_perview2(std::vector<float> &ndotv, vector<float> &kr)
{
...
#pragma omp parallel for
    ...
    {
        //从背面投影
        vec back_viewpos = 2.0f * triMesh->bsphere.center - viewpos;


        vec viewdir = back_viewpos - triMesh->vertices[i];
...
//        QString ocfilename = "oc.txt";
//        saveOccludingFile(ocfilename);
...
void TriMeshView::smooth_curv()
{
    ...
    diffuse_curv(triMesh, currsmooth);
    int time_Diff = time.elapsed(); //返回从上次start()或restart()开始以来的时间差，单位ms

    //以下方法是将ms转为s
    float f = time_Diff/1000.0;
    printf("smooth_time: %f\n", f);fflush(stdout);

    triMesh->dcurv.clear();
    triMesh->need_dcurv();

//    write_to_file(triMesh->curv1, "normal_curv1_smooth.txt");
//    write_to_file(triMesh->curv2, "normal_curv2_smooth.txt");

    curv_colors.clear();
    currsmooth *= 1.1f;
}

void TriMeshView::time_ridge_valley()
{
    QTime time;
    ...
    isSavedRV = true;
    draw_mesh_ridges(true, vector<float>(), true, true,rv_thresh / feature_size); //ridges
    draw_mesh_ridges(false, vector<float>(), true, true,rv_thresh / feature_size);//valleys
    isSavedRV = false;

    int time_Diff = time.elapsed(); //返回从上次start()或restart()开始以来的时间差，单位ms
    //以下方法是将ms转为s
    float f = time_Diff/1000.0;
    printf("test_ridge_valley_time: %f\n", f);fflush(stdout);

}

void TriMeshView::smooth_dcurv()
{
    printf("\r");  fflush(stdout);
    diffuse_dcurv(triMesh, currsmooth);
    currsmooth *= 1.1f;
}

void TriMeshView::laplace_smooth_curv()
{
    printf("\r");  fflush(stdout);
    zdd_diffuse_curv(triMesh, currsmooth);
    triMesh->dcurv.clear();
    triMesh->need_dcurv();
...
void TriMeshView::saveCurv1(const QString &fileName)
{
    write_to_file(triMesh->curv1, fileName);
}

void TriMeshView::saveCurv2(const QString &fileName)
{
    write_to_file(triMesh->curv2, fileName);
}
