...
void rot_coord_sys(const vec &old_u, const vec &old_v,
                          const vec &new_norm,
                          vec &new_u, vec &new_v)
{
    new_u = old_u;
...
#pragma omp parallel for
    ...
    dprintf("Done.\n");
    dprintf("curv1.size() %d .\n", curv1.size());
    dprintf("pdir1.size() %d .\n", pdir1.size());

    ofstream outfile;
    outfile.open("pv1.txt", ios::trunc);
    ...
    outfile.open("pv2.txt", ios::trunc);
    ...
    outfile.open("pd1.txt", ios::trunc);

    for (int i = 0; i < nv; i++) {
       outfile << pdir1[i][0] << " " << pdir1[i][1] << " " << pdir1[i][2]  << "\n";
    }
    outfile.close();

    outfile.open("pd2.txt", ios::trunc);

    for (int i = 0; i < nv; i++) {
       outfile <<  pdir2[i][0] << " " << pdir2[i][1] << " " << pdir2[i][2]  << "\n";
    }
    outfile.close();
